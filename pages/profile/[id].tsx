...
const Profile = ({ data }: IProps) => {
  ...
  useEffect(() => {
    if (showUserVideos) {
      ...
      setVideoList(data.userLikedVideos);
    }
  }, [showUserVideos, setVideoList, data.userVideos, data.userLikedVideos]);
    ...
    <div className='w-full'>
      <div className='flex gap-6 md:gap-10 mb-4 bg-white w-full'>
        ...
        <div className='flex flex-col justify-center'>
          ...
          <p className='capitalize text-gray-400 text-xs md:text-lg'>
            {data.user.userName}
          </p>
        </div>
      </div>
      <div>
        <div className='flex gap-10 mt-10 mb-10 border-b-2 border-gray-200 bg-white w-full '>
          ...
          <p
            className={`text-lg font-semibold cursor-pointer mt-2 ${liked}`}
